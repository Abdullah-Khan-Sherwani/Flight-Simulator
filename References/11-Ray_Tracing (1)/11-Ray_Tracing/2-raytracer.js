...
/**
 * A point light source with separate Phong colour channels and
...
class Light {
     ...
     * combining attenuation with all three Phong lighting terms.
     *
     *   radiance = attenuation * (ambient + diffuse + specular)
     *
     *   ambient  = Ka * La
     *   diffuse  = Kd * Ld * max(0, N·L)
     *   specular = Ks * Ls * max(0, V·R)^shininess
     *
     * @param  {Object}   hit     - Hit record { coord, normal, obj }.
     * @param  {Material} mat     - Surface material (ambient, diffuse, specular, shininess).
    ...
    radianceAt(hit, viewDir) {
        ...
        if (NdotL > 0) {
            ...
            // Specular
            const incident = glMatrix.vec3.create();
            glMatrix.vec3.scale(incident, toLight, -1);
            const reflectDir = glMatrix.vec3.create();
            reflect(reflectDir, incident, hit.normal); // 
 ...
 *
 * Note: ambient is intentionally included inside radianceAt() and therefore
 * only applied when the light is NOT in shadow.  To have ambient reach even
 * shadowed surfaces, move the ambient term outside the shadow test in
 * radianceAt(), or compute it separately here.
 *
 * @param  {Object} scene
...
function shade(scene, ray, hit) {
    ...
    for (const light of scene.lights) {
        // Build shadow ray from hit point toward the light.
        // Direction and distance are computed inside isLightVisible via a
        // temporary ray; we need them here too for the shadow origin.
        const toLight = glMatrix.vec3.create();
        ...
        glMatrix.vec3.scaleAndAdd(shadowOrigin, hit.point, hit.normal, 1e-4);
 ...
 *
 * Each Material takes (ambient, diffuse, specular, shininess, reflectivity).
...
// Instantiate an 800×600 camera and kick off the synchronous render.
