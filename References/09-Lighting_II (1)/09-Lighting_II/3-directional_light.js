...
window.onload = function init() {
    ...
    const lightDirection = glMatrix.vec3.fromValues(-0.2, -1.0, -0.3);

    // init transformations for cube and lamp
    MVPtransforms.model = glMatrix.mat4.create();
    ...
    // set up event listener fors moving camera 
...
function render(timestamp) {
    ...
    for (let i = 0; i < cubePositions.length; i++) {
        glMatrix.mat4.fromTranslation(MVPtransforms.model, cubePositions[i]);
        glMatrix.mat4.rotate(MVPtransforms.model, MVPtransforms.model, glMatrix.glMatrix.toRadian(20.0 * i), [1.0, 0.3, 0.5]);
        gl.uniformMatrix4fv(uniforms.modelCube, false, MVPtransforms.model);
